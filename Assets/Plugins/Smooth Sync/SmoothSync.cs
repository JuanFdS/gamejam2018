...
namespace Smooth
{
    ...
    public class SmoothSync : NetworkBehaviour
    {
        ...
        /// <summary>Set to 0 to always send the position of owners if it has changed since the last sent position, and for a hardware 
        ...
        /// <summary>Set to 0 to always send the rotation of owners if it has changed since the last sent rotation, and for a hardware 
        /// performance increase, but at the cost of network usage.
        ...
        /// <summary>Set to 0 to always send the velocity of owners if it has changed since the last sent velocity, and for a hardware 
        /// performance increase, but at the cost of network usage.
        ...
        /// <summary>Set to 0 to always send the angular velocity of owners if it has changed since the last sent angular velocity, and for a hardware 
        /// performance increase, but at the cost of network usage.
        ...
        /// <summary>Set to 0 to always update the position of non-owners if it has changed, and for a hardware performance increase.
        ...
        /// Usually keep this at 0 or really low, but it's useful if you are extrapolating into the future and want to stop instantly 
        /// and not have it backtrack to where it was.
        /// Measured in distance units.</summary>
        ...
        /// <summary>Set to 0 to always update the rotation of non-owners if it has changed, and for a hardware performance increase.
        ...
        /// Usually keep this at 0 or really low, but it's useful if you are extrapolating into the future and want to stop instantly and 
        /// not have it backtrack to where it was.
        /// Measured in degrees.</summary>
        ...
        void Awake()
        {
            ...
            // to the send rate and how far back in time we want to be. Doubled buffer for custom state updates.
        ...
        void Update()
        {
            ...
            {
                setInterpolationOrExtrapolationPosition();
            }
        ...
        void setInterpolationOrExtrapolationPosition()
        {
            ...
            if (!triedToExtrapolateTooFar || (!hasRigdibody && !hasRigidbody2D))
            {
                ...
                float distance = 0;
                // If current position is different from target position or if we want to 
                // check with receivedMovementThreshold, calculate the distance.
                if (getPosition() != targetState.position || receivedMovementThreshold > 0)
                {
                    distance = Vector3.Distance(getPosition(), targetState.position);
                }

                if (receivedMovementThreshold > 0)
                {
                ...
                // If current rotation is different from target rotation or if we want to 
                // check with receivedRotationThreshold, calculate the difference.
                if (getRotation() != targetState.rotation || receivedRotationThreshold > 0)
                {
        ...
        /// Full example of use in the example scene in PlayerController.cs.
        /// </remarks>
        ...
        /// <summary>
        /// Forces the state to be sent on owners the next time it goes through Update().
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        /// <remarks>
        /// If sendMovementThreshold is 0, returns true if the current position is different than the latest sent position.
        /// If sendMovementThreshold is greater than 0, returns true if distance between position and latest sent position is greater 
        /// than the movement threshold.
        /// </remarks>
        public bool shouldSendPosition()
        {
            if (forceStateSend ||
                (getPosition() != lastPositionWhenStateWasSent &&
                (sendMovementThreshold == 0 || Vector3.Distance(lastPositionWhenStateWasSent, getPosition()) > sendMovementThreshold)))
            {
                return true;
            }
            else
